-- Stage C: analysis-table SELECT authorization
-- Replaces authenticated USING (true) on practices / doctors / affiliations
-- with is_atlas_analysis_authorized():
--   auth.uid() present
--   AND matching profiles row
--   AND deleted_at IS NULL
--   AND (onboarding_complete IS TRUE OR is_admin IS TRUE)
-- is_internal is intentionally ignored.
--
-- practice_locations intentionally unchanged (Stage B authenticated SELECT retained).
-- No Stage A public RPCs / anon revokes.
